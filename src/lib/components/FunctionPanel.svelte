<script lang="ts">
  ...
  function deleteFunction(idToDelete: number) {
    functions.update((currentFunctions) =>
        ...
        <div class="table-row">
          ...
          <div class="col name">
            <input
              ...
              placeholder="e.g., rate1"
              maxlength="8"
              on:input={(e) => {
                const v = e.currentTarget.value || "";
                if (v.length > 8) e.currentTarget.value = v.slice(0, 8);
              }}
            />
          </div>
          ...
          <div class="col action">
            <button
              class="delete-btn"
              aria-label={`Delete function ${func.name || func.id}`}
